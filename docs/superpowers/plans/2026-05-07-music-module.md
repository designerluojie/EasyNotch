...
- [ ] **Step 3: Implement the capability, snapshot, session, collapsed-summary, and state types**
...
final class AppCompositionRoot: ObservableObject {
    ...
    init(
        ...
        self.musicRuntime = musicRuntime ?? MusicModuleRuntime(sharedServices: self.sharedServices, energyGovernor: self.energyGovernor)
        self.moduleRuntimeRegistry = ModuleRuntimeRegistry.defaultRegistry(overrides: [.music: self.musicRuntime])
    }
}
```

- [ ] **Step 4: Make `NotchShellRuntime` and `ContentHostView` consume the shared runtime**
...
struct ActiveMusicSessionResolver {
    func resolve(_ snapshot: MusicPlayerSnapshot?) -> MusicPlayerSnapshot? {
        ...
        guard snapshot.capability.phase == .verified else { return snapshot }
        return snapshot
    }
}
