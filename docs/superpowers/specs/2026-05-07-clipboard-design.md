...
- 单击历史项即可触发写回
- 点击历史项写回成功后收起面板，并将该历史项前移到列表首位
- Figma 图形、Figma 文字、文件引用以真实可回写为目标，不接受只识别展示不支持回写
- 图片不统一转为 PNG，必须保留源格式语义并按原始表示回写
- 文件 / 图片引用项采用“回写真值与展示快照分离”的策略：bookmark 负责回写，缩略图快照负责稳定展示
...
- `thumbnailLocation` 指向展示缩略图快照，只用于展示，可选且可重建
...
- 允许另外生成展示用缩略图快照，但缩略图不参与真实回写

说明：

- 直接复制的图片内容通常不是引用型，而是内嵌图片数据
- 这类图片可以直接从已存 payload 生成缩略图，不存在“引用失效”问题
...
- 对文件引用类项，展示层使用“缩略图快照 + 当前有效性状态”语义
- 缩略图应尽量代表复制当时的样子，而不是每次打开时重新反映当前文件内容
...
元数据写入 `history.json`，真实 payload 分类型写入 `Payloads`。缩略图快照写入 `Thumbnails`，并且必须与原始 payload 分离。
...
- 默认显示复制当时生成的缩略图快照
- 若当前引用失效，则保留原缩略图快照并在右上角叠加问号徽标
- 只有在没有任何可展示快照时，才退回问号占位态
...
4. 用户点击某条历史项
5. 对引用型内容先做严格有效性校验
6. `PasteExecutor` 恢复原始 payload 并写回系统剪贴板
7. 写回成功后将该项前移到列表首位
8. 写回成功后收起当前面板
...
- 不制造假成功态
- 在当前页给出明确失败反馈
- 对引用型内容再次确认失效时，要明确提示“引用已失效”而不是静默失败
...
对文件引用类项，即使缩略图生成失败，也不应阻断历史入库；该项可以先以文本或系统图标态展示，后续在可见时再尝试补建快照。
...
对文件引用失效项，展示允许保留历史缩略图快照，但点击时必须再次严格校验并明确失败。
