...
struct ClipboardNormalizer {
    ...
    init(thumbnailService: ClipboardThumbnailService = ClipboardThumbnailService()) {
    ...
    nonisolated private static func firstInlinePayload(
        ...
        for type in snapshot.availableTypes where predicate(type) {
            if let data = snapshot.dataByType[type] {
                return (type, data)
            }
        }

        for (type, data) in snapshot.dataByType where predicate(type) {
            return (type, data)
        }
    ...
    nonisolated private static func makeFigmaCapture(
        ...
        guard let (_, htmlData) = firstInlinePayload(in: snapshot, matching: isHTMLType),
        ...
        let representations = snapshot.availableTypes.compactMap { pasteboardType -> ClipboardInlineRepresentation? in
            guard let data = snapshot.dataByType[pasteboardType] else {
                return nil
            }
